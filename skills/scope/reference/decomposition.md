...
When a task is too big for one PR or one context window, make it a parent folder and split it into numbered sub-task folders (`01-slug/`), each with its own `STATE.md` and `requirements.md`. Sub-tasks run in parallel; add `depends_on` (a sub-task's number) to a sub-task's `STATE.md` only where one truly needs another's result, and fix any shared schema or API in the parent's `requirements.md` first, or make it an earlier sub-task the rest depend on. Compare sibling `paths` before writing anything: two sub-tasks whose paths overlap are merged, or one takes a `depends_on`. Each sub-task restates `out_of_scope` in its own `STATE.md`. The numbered folders are the list — the parent keeps no index, and status is read from each sub-task's frontmatter. The no-dependencies rule above is for the actionable list inside one task, not for splitting into sub-task folders.
