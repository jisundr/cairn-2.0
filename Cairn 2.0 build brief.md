...
Report to a self-contained HTML file — no server, no CDN — with per-day, per-session, and per-agent rollups and a visible warning when any usage-limit event was recorded.
