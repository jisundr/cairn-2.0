...
2. **Implement** — `cairn:builder`, following `commands/cairn-doctor.md`'s frontmatter/body style:
...
3. **Gate** — full §A13 gate + relevant §B12 acceptance criteria (specifically #26: token report opens correctly from `file://` — recheck against the live-server amendment, since the original brief assumed a static HTML file and this ships a live server instead).
4. **Manual test** — the feature's full end-to-end acceptance, per `../03-architecture.md`'s Testing section and `../GOAL-CONDITION.md`'s overall Done-when list:
...
5. **Confirm tests** — manual only, per `../ROADMAP.md`'s M6 test note; no new automated coverage beyond what M1–M5 already carry.
6. **PR** — `cairn:review-pr`, diff scoped to `commands/cairn-tokens.md`.
