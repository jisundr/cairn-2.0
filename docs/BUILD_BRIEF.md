...
**Amended, then re-amended:** implementation code (`db.py`, `parser.py`, `pricing.py`, `server.py`, `frontend/`) briefly lived in a separate `token-metering` git submodule instead of `tools/tokens/` in this repo — a fresh `/plugin install` doesn't recurse git submodules, which left `/cairn-tokens` with no backend to start. The backend now lives at `tools/tokens/` as regular tracked files in this repo; only `frontend/` (the dev-time React/Vite build, never a consuming-project runtime dependency) and its Node/npm build tooling stay in the `token-metering` submodule, maintainers-only. `tools/budget.py`'s walk and `python -m pytest tools/` now cover `tools/tokens/` directly; an installed checkout with the submodule absent or uninitialized is fully supported, since nothing at runtime reads there. Sequencing: `docs/features/token-metering/ROADMAP.md`. Relocation record: `docs/tasks/vendor-token-metering-backend/`.
...
2. Token metering — backend implementation lives at `tools/tokens/` in this repo (see B10's re-amendment), sequenced by `docs/features/token-metering/ROADMAP.md`'s milestones; `hooks/stop-tokens.sh` and `commands/cairn-tokens.md` land in this repo alongside it; only `frontend/` stays in the `token-metering` submodule.
