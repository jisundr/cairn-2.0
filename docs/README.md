...
Per-feature doc folders, one per shipped or in-progress feature. Currently: `token-metering/` — the Phase 2 token-metering feature, starting from `01_intent.md` (why it exists, in one page) through `02_requirements.md`, `03_architecture.md`, and `user-flow.md`, plus a `mockups/` dashboard.
