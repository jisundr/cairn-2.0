...
3. **Classify** — first match wins:
   ...
   - **Ready to close** — standalone `done`/`Done` in `key_info` (not `done_when`, not "not done"). Not direct-commit: also `gh pr list --head <folder-name> --state merged --json number -q 'length'` = `1`; output other than `0`/`1` → go by `key_info`. Direct-commit or `research` folder: skip `gh`.
